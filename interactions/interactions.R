...
#' @param merge character that names method of merging factor levels.
#' "None" for no merging, "FactorMerger" for merging based on Merging Path Plot,
#' "Rare" for merging based on proportions (rare levels will be merged).
#' If "factorMerger" was chosen, response argument must be non-NULL, 
#' if "Rare" was chosen, cutoff argument must be non-NULL.
#' @param response Response variable which will be used if merging will be based
#' on preditictive qualities.
#' @param cutoff proportion
#' 
#'

get_interactions <- function(x, exclude = NULL, merge = "None", response = NULL, cutoff = NULL) {
...
merge_rare_levels <- function(x, exclude = NULL, min_n = NULL, min_prop = NULL) {
