...
count_events <- function(x, case_id_col = "spr_Id", 
  ...
  results <- data.table::rbindlist(all_cases, fill = T, use.names = T)
  if(all(c("combined_names", "value") %in% colnames(results))) {
    results[, -..c("combined_names", "value")]
  } else {
    results
  }
}
...
#' 
#' This is parallelized version of count_events function. To use it properly, register a cluster using
#' registerDoParallel before running it (and stop the cluster afterwards).
#'
#' @param x Data frame
#' @param case_id_col Character, name of the column in which case IDs are stored
#' @param event_date_col Character, name of the column where event dates are stored
